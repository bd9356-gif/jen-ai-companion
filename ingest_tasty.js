// Tasty API Recipe Ingestion Script - Round 2
...
// New tags not used in round 1
const TAGS = [
  'under_30_minutes', 'one_pot_or_pan', 'meal_prep', 'kid_friendly',
  'date_night', 'party', 'brunch', 'sides', 'french', 'japanese',
  'caribbean', 'middle_eastern', 'greek', 'korean', 'vietnamese',
  'thai', 'indian', 'chinese', 'spanish', 'moroccan',
  'turkey', 'lamb', 'duck', 'vegetable', 'rice', 'noodles',
  'sandwich', 'tacos', 'pizza', 'steak', 'shrimp', 'salmon',
  'slow_cooker', 'instant_pot', 'sheet_pan'
]
...
function mapDietaryTags(tags) {
  ...
  return (tags || []).map(t => t.name).filter(t => dietary.includes(t))
}
...
async function ingestTag(tag) {
  ...
  for (const r of recipes) {
    if (!r.name || !r.instructions?.length || !r.sections?.length) { skipped++; continue }
    ...
    const { data: recipeRow, error: recipeError } = await supabase
      ...
      .upsert({
        ...
        source_url: r.slug ? `https://tasty.co/recipe/${r.slug}` : '',
        tags,
        ingredients,
      }, { onConflict: 'title' })
      .select('id')
...
async function main() {
  console.log('🍳 MyRecipe Companion — Tasty API Ingestion Round 2')
  console.log('====================================================')
