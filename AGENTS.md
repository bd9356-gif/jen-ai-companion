...
- **Clipboard auto-jump on Vault open (April 2026).** Bill's pivot from the iOS Shortcut path: the Shortcut UX was too fiddly to set up (URL Encode action, Open URL action, share-sheet enrollment), and the existing copy-paste flow already worked. To collapse the copy-paste path from 4 taps to 2, `loadRecipes` now reads `navigator.clipboard.readText()` after the auth load, and if the clipboard holds an `http(s)://...` URL (length ≤ 2000, not pointing at our own domain), it auto-switches `view` to `'import'`, sets `importTab='url'`, and pre-fills `importUrl`. The user lands on the Import view with the URL already in the field — one tap on **📥 Import & Clean with AI** finishes the job. Gated on `!recipeParam && !importParam` so explicit deep-links always win, and on the same domain-skip regex (`recipe.mycompanionapps.com`, `jen-ai-companion.vercel.app`) so copying a Vault link doesn't try to import itself. Import is **not** auto-fired — pre-fill only — so a stale URL from yesterday's copy can't silently re-run. `readText()` may be denied by Safari/iOS without a recent user gesture; the catch-arm falls through silently and the existing in-Import-view clipboard prompt still catches the URL on the next tap, so the regression-floor is "current behavior."
