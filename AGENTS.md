...
- **Collapsible search input (list view).** The text search is hidden by default behind a 🔍 icon button that sits to the right of the chip row. Tapping expands the input (with `autoFocus`); tapping again collapses it. If a `searchText` value is present the input stays visible regardless of the toggle so the active search is always obvious. The 🔍 button itself turns orange-filled when `showSearch || searchText` to signal the search is engaged.
