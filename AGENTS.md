...
- **Clipboard auto-jump on Vault open (April 2026).** Bill's pivot from the iOS Shortcut path: the Shortcut UX was too fiddly to set up (URL Encode action, Open URL action, share-sheet enrollment), and the existing copy-paste flow already worked. To collapse the copy-paste path from 4 taps to 2, `loadRecipes` calls the `readClipboardSmart()` helper after the auth load and branches on what it finds. **`readClipboardSmart()` uses `navigator.clipboard.read()` first** (the richer Async Clipboard API that exposes every MIME type on the clipboard) and falls back to `readText()` when `read()` is unavailable. This matters because iOS Shortcuts' "Get Contents of Web Page" action puts **HTML / rich text** on the clipboard, not plain text — `readText()` returns empty or a useless snippet, but `read()` lets us pull `text/html`, strip tags into a clean text dump (`htmlToCleanText()` — script/style blocks dropped, block-level tags turned into newlines, remaining tags stripped, common HTML entities decoded, whitespace collapsed), and feed that into the auto-jump logic. **Longer-wins between text/plain and text/html.** When BOTH MIME types are present (the common case on iOS Safari's Share → Copy and Shortcuts' Get Contents of Web Page), the helper returns whichever is longer. iOS web-page clipboards typically put a short URL fallback on `text/plain` and the actual page content on `text/html` — preferring `text/plain` first (the obvious-looking choice) routed users to the URL tab when they wanted Paste; the longer-wins rule fixes that without losing the URL-only case (text/plain wins when it's the only thing present, or when text/html happens to be smaller). Helper is at module scope in `app/secret/page.js` so a future Recipe Cards / Chef Notes paste path can reuse it.
  - **Branch A+ — URL+HTML payload** (matches `^\s*(https?:\/\/\S+)\s*\r?\n\s*\r?\n([\s\S]+)$` — Bill's iOS Shortcut clipboard format): auto-switch to `view='import'`, `importTab='url'`, pre-fill `importUrl` with **just the URL line**, and stash the HTML half in a new `pendingHtml` state. `handleImport` reads `pendingHtml` as the `htmlOverride` fallback (and clears it after each run), so the URL fetch runs server-side as the primary path (gets canonical og:image on A-sites) and the API silently falls through to the stashed HTML on B-site failures (still gets an image from the page contents). This branch comes BEFORE Branch B because the HTML body almost always matches the recipe vocabulary in Branch B's gate, which would otherwise route the whole payload to Paste tab and lose the URL → image extraction path. The user just sees "URL tab with my recipe URL pre-filled" — same UX as Branch A — and the B-site rescue is invisible.
  ...
  - **Branch B — recipe-shaped text on clipboard** (RETIRED April 2026). Used to auto-route long page-text dumps to the Paste tab pre-filled. Removed because the routing combined badly with the iOS Shortcut path: when iOS's paste-tray suggestion fires inside a pre-populated textarea it overwrites the URL line on top with the rich-text body, and the import runs text-only with no image — exactly the failure mode that motivated smart_import in the first place. URL is now the only path the auto-jump routes to. If the clipboard is just text (no URL), the user stays on the recipe list and explicitly opens 📥 Import → Paste themselves. Paste tab is the deliberate fallback for B-sites, not a clever default.
  - **Branches are gated on** `!recipeParam && !importParam` so explicit deep-links always win. Import is **not** auto-fired — pre-fill only — so stale clipboard content can't silently re-run. `readText()` may be denied by Safari/iOS without a recent user gesture; the catch-arm falls through silently and the existing in-Import-view clipboard prompt still catches a URL on the next tap, so the regression-floor is "current behavior."
  - **Gesture-triggered fallback — `openImportFromClipboard()` on the 📥 Import button.** iOS Safari refuses `navigator.clipboard.read()` on page load because there's no recent user gesture, so the on-load auto-jump above silently denies on iPhone. The 📥 Import buttons (in the sticky header AND on the empty state) are wired to a wrapper that calls `setView('import')` first, then runs `readClipboardSmart()` and applies the same Branch A+ / Branch A routing. Tapping the button IS the gesture, so iOS will allow the clipboard read and prompt the user with its native "Paste from <App>?" sheet. Once they confirm, we route to URL tab and pre-fill if there's a URL on the clipboard; otherwise Import opens on URL tab with empty fields and the user can navigate to Paste tab manually. The button never feels broken — Import always opens.
