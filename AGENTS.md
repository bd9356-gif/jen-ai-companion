...
These live in `app/kitchen/page.js`. MyKitchen is **2 sections and 8 tiles** total (after the April 2026 Cook / Learn Your Way reframe). All tiles use a unified **orange left stripe** (brand color) — `border-2 border-gray-200 border-l-8 border-l-orange-600 hover:border-orange-300 hover:shadow-sm rounded-2xl` — mirroring Golf's green-stripe Clubhouse pattern.

Section headers are small orange uppercase labels with a one-line section subtitle below, followed by the section's tiles.

### Section structure

1. **Cook** — "Recipes, meal plans & shopping, simplified by AI." Everything the user *does* in the kitchen — saving recipes, planning meals, shopping for them. Plain tile names ("Recipe Vault", "Meal Plan").
...
**Tile description rhythm.** Cook descriptions stay short (~25–30 chars) so they fit one phone line under `truncate` — short, punchy, scannable, mirroring Golf Clubhouse's tile rhythm ("Every lesson, one tap away." / "Notes, tips, tee times."). Learn Your Way descriptions are slightly longer one-liners that describe what the surface DOES — the Cook rhythm doesn't fit teaching surfaces, where the action is the value (e.g. "Core knife skills, techniques, and cooking fundamentals." reads better than a 25-char tagline that has to drop the substance). The earlier "Name — Role" pattern descriptions (e.g. "Learn directly from Chef Jennifer — your personal AI cooking teacher.") were 58–70 chars and `truncate`'d on iPhone — the current descriptions are tuned to fit without truncation while still saying something concrete.

**Reframe history (April 2026).** The hub used to read as three flat groups (Your Recipes / Plan & Shop / Cooking School). Once Chef Jennifer + Chef TV + Library + Playbook were all locked together under one Learning section, the symmetry tipped — *all* the cooking-life surfaces (saving + planning + shopping) belong together too. So the hub became two halves of the same story: the *do* side (Recipe Vault + Recipe Cards + Meal Plan + Shopping List) and the *learn* side (Chef Jennifer + Chef TV + Library + Playbook). The do-side was first labeled **Cooking Life** ("Your recipes, your plan, your essentials.") and renamed to **Cook** ("Recipes, meal plans & shopping, simplified by AI.") — a single verb reads as a clearer counterpart to a learning section, and the "simplified by AI" tag telegraphs the differentiator on first glance. The learn-side traveled "Cooking School" → "Learning Journey" → "Learn Your Way / An AI-powered cooking school." for the same reason — concrete framing beat abstract metaphor. Inside Learn Your Way, Chef Jennifer was promoted from second tile (after Chef TV) to first — she's the AI instructor and the most personal teaching surface, so she leads the column.

**Title format note.** Both groups use plain tile names — no "Name — Role" pattern. Cook tiles ("Recipe Vault", "Meal Plan") have always been plain because *what they are* and *what they do* collapse to the same thing. Learn Your Way tiles ("Chef Jennifer", "Chef TV", "Your Library", "My Playbook") were briefly em-dashed ("Chef Jennifer — Your Instructor", "Guides — Your Library") to carry teaching framing the plain names couldn't, but the role-labels added visual noise without enough payoff once the descriptions started doing the work — reverted to plain names (April 2026). The "Guides" tile was also renamed to **"Your Library"** in the same pass (route stays `/guides`) because "Your Library" reads as the *thing it is* on the hub, while "Guides" needed the em-dash role-label to communicate.
...
`loadAll` fetches the three video source tables + `cooking_skill_items` + AI-answer favorites in parallel and merges them. If the same underlying video exists in both legacy (`saved_videos`) and new (`favorites`) form, the legacy row wins (dedupe by `_item_type:_item_id`). Each video row renders via `<VideoItem>` plus a context-dependent action button below the thumb. **In 🎓 Teach:** "Move to 🍳 Practice" — upserts into `cooking_skill_items` and auto-switches the active tab to the destination. **In 🍳 Practice:** "🔐 Move to Recipe Vault" (April 2026) — fetches the video's metadata (from `video_metadata` for legacy items or `favorites.metadata` for favorites-sourced items) and inserts a row into `personal_recipes` with title/ingredients/instructions/ai_summary plus a `family_notes: "Saved from Chef TV — {channel}."` attribution and the YouTube hqdefault thumbnail as `photo_url`. The video stays in Practice (this is a cross-surface save, not a bucket move). Mirrors the 💾 Save to My Kitchen button on Chef TV's Recipe view, surfaced inside Playbook so users browsing their saved Practice videos can promote the keepers to the Vault without leaving the page. Removing a row deletes from the source table AND the `cooking_skill_items` row. A slate callout above the tabs explains each surface in one sentence.

**Column headers are tappable** (April 2026). The `👨‍🍳 Chef Jennifer` and `🎬 Chef TV` labels above each column of the 2×2 pill grid are buttons — Chef Jennifer goes to `/chef`, Chef TV goes to `/videos`. Lets users jump from Playbook back to the source surface where they generated the saves they're reviewing. Hover/tap state tints them orange to telegraph they're clickable.
...
- **Topic chips — per-tab shortcut sets (no channel dropdown).** The old full-width `<select>` channel dropdown is retired. Below the pill row, Teach and Practice each show their own horizontally-scrollable row of topic chips. One chip active at a time; active chip uses the tab's color (sky for Teach, orange for Practice); inactive chips are white with a gray border. Chip filter is a regex match against the video **title** AND (on Practice videos with recipe metadata) the recipe's **ingredients** list — title-vague videos like "Crispy Wings 3 Ways" still surface under 🍗 Chicken because the ingredient list names the protein. We don't have a proper tag/topic column on `cooking_videos`, so the regexes are keyword-based and tuned empirically; `\b…\b` word boundaries on Meat/Chicken/Fish keep the chips from cross-matching (a "chicken pot pie" video lands in Chicken, not Meat). Constants in `app/videos/page.js`:
  - `TEACH_CHIPS`: ⭐ Featured · All · 🔪 Knife · 🥚 Eggs · 🥩 Meat · 🍗 Chicken · 🍞 Baking · 🧂 Season · 📚 Basics. Default: Featured.
  - `PRACTICE_CHIPS`: ⭐ Featured · All · 🍝 Pasta · 🍕 Pizza · 🥗 Salad · 🍲 Soup · 🥩 Meat · 🍗 Chicken · 🐟 Fish · 🍞 Bread · 🍰 Sweet. Default: Featured. The Featured chip shows is_featured rows only — same strict-curator filter as Teach's Featured chip.
  ...
  - **⭐ Featured** is a special chip on both Teach AND Practice (not a keyword filter). It shows **only** the videos the curator has marked `is_featured = true` — no fillers, no cap. All other chips apply keyword matching to their tab's full set. (Earlier behavior padded the chip with the auto top-by-score slice when there weren't yet 15 featured rows; retired April 2026 because it turned the chip into a "best of" view instead of a curator-only view.)
...
- **`personal_recipes`** structured columns (May 2026, migration 015) — `prep_time_minutes int, cook_time_minutes int, total_time_minutes int, calories int, protein_g numeric(6,2), carbs_g numeric(6,2), fat_g numeric(6,2)`. All nullable; populated by `parseRecipeSchema` in `/api/import-recipe` when the page ships JSON-LD Recipe markup. Vault detail view renders a pill row at the top + a "📊 Recipe Info" expandable showing whatever fields are set. The Add form has editable inputs for all seven so users can correct or fill them in manually. Older Vault rows that came in without JSON-LD just have these columns null — the conditional rendering hides empty pills.
...
- **Import Tools — four tabs (URL / Paste / Add / JSON) (April 2026).** The Import view was renamed **📥 Import Tools** and grew a fourth tab. `importTab` state values: `'url' | 'paste' | 'add' | 'json'`, default `'url'`. Four equal-width pill buttons in a `bg-gray-100 rounded-2xl p-1 grid-cols-4` container; active tab fills `bg-orange-600 text-white`, inactive tabs are `text-gray-600`. Below the tabs only the active tab's card renders.
  - **🔗 URL** — primary path. Paste a recipe URL, tap Import. Server fetches HTML, extracts og:image + JSON-LD recipe, lands on Add tab pre-filled.
  - **📋 Paste** — fallback for B-sites that block the fetcher. Has three collapsible **option groups** above the textarea (📝 Paste Text / 🖨️ Print Capture / 📲 Share Shortcut) — each is just instructions for HOW to get content into the textarea below; all three end at the same paste step. State `pasteOption` in `app/secret/page.js` tracks which group is expanded (`null` | `'text'` | `'print'` | `'shortcut'`). Tap a header to expand/collapse; multiple closed by default so the page is short on first open. The textarea + `📋 Paste from clipboard` button + Import button live below the option groups.
  - **✏️ Add** — manual entry + post-import preview surface. Form fields (Photo / Title / Description / Category / Tags / Ingredients / Instructions / Family Notes) live inline as the tab content with their own Cancel + 💾 Save Recipe row at the bottom. The standalone `view === 'add'` page was retired April 2026 — Add lives entirely inside Import Tools now. Same form is used for fresh manual entry AND for the post-import preview: when URL/Paste/JSON imports succeed, `setImportTab('add')` flips here with the form pre-filled. The Vault's old + Add header button was removed at the same time; entry to Add is exclusively via Import Tools (the 📥 button in the Vault header opens Import Tools, then user picks the Add tab).
  - **📄 JSON** — power-user import/export. File picker for JSON exported from this app or any compatible format. Has its own button inside the card (no shared Import button).
  - URL/Paste share the bottom **📥 Import & Clean with AI** action; Add has its own 💾 Save row; JSON has its own file picker. The shared error banner only renders on URL/Paste and suggests switching to Paste with a clickable inline button.
- **Import view — clipboard auto-detect on the URL field (RETIRED April 2026).** Used to peek at `navigator.clipboard.readText()` whenever the user landed on the URL tab and surface a `📋 URL on your clipboard — Use it` prompt. Removed because the app-side clipboard read triggered iOS's "Paste from <App>?" prompt every time the user opened Import, even when the clipboard was unrelated. Simpler now: user pastes URL themselves into the field — iOS shows its native paste suggestion above the keyboard at the right moment (when the input is focused), without the app polling on entry. The explicit `📋 Paste` button next to the URL input is kept (it's a user-gesture clipboard read, no on-entry polling) so users who prefer a button still have one. State `clipboardSuggestion` and the in-effect `useEffect([view, importTab, importUrl])` were both removed.
...
- **Recipe import deep-link — `?import=<encoded-url>` and `/import?url=<encoded-url>` (April 2026).** External entry point for "send a URL to MyRecipe" flows — primary use case is the iOS Share-Sheet shortcut Bill builds in iOS Shortcuts (Safari → Share → "Send to MyRecipe").
  ...
  - **Why a separate `/import` page** instead of just `/secret?import=…`: the URL is shorter and easier to type into a Shortcut, gives us one place to centralize auth-redirect-with-preserved-URL, and reserves room for future ingestion entry points. The HTML-blob-bypass-the-scraper path mentioned here previously is now wired up — see the smart_import bullet below.
- **Smart import — `?smart_import=1` + clipboard `URL\n\nHTML` payload (April 2026).** The iOS Share-Sheet "Send to MyRecipe" Shortcut path. One share-sheet tap, recipe lands — no choosing URL vs Paste, no thinking about whether the site blocks our scraper. Works on A-sites (allow our fetch) and B-sites (block our fetch) without the user knowing the difference.
  - **The Shortcut (5 actions on iOS).** (1) Receive URLs from Share Sheet. (2) Get Contents of Web Page from Shortcut Input — produces page HTML. (3) Text action: `[Shortcut Input]\n\n[Contents of Web Page]` — combines URL on first line, blank line, HTML below. (4) Copy to Clipboard. (5) Open URL → `https://recipe.mycompanionapps.com/secret?smart_import=1`.
  - **The page handler (`/secret?smart_import=1`).** `loadRecipes` reads the param early, switches to `view='import'` + `importTab='url'`, strips the param from history (so refresh doesn't re-trigger), then on next tick calls `readClipboardSmart()` (the existing helper that handles iOS rich-text clipboards). The payload is parsed with a tolerant regex: `^\s*(https?:\/\/\S+)\s*\r?\n\s*\r?\n([\s\S]+)$` — first line is the URL, blank line, then HTML. Falls back gracefully: URL-only payload (no separator) → URL-only POST; long blob with no leading URL → HTML-only POST; empty clipboard → leaves Import view ready for manual entry. Once parsed, calls `handleImport(url, html)` which POSTs both to `/api/import-recipe`.
  - **The API decides which path won.** `/api/import-recipe` accepts an optional third param `html` alongside the existing `text` / `url`. When `url` is present, the route tries the server-side fetch first (lighter, gets canonical og:image, works on A-sites). If that fetch fails (HTTP 4xx/5xx, network error, timeout) AND `html` is present, it silently falls through to extracting content from the supplied HTML (B-site rescue). User never sees which path won — same response shape either way. The shared HTML→content extractor (`extractContentFromHtml`) prefers JSON-LD Recipe blocks, falls back to stripping all HTML to plain text. Image extraction (`extractImageUrl`) runs on whichever HTML we end up using, with the original URL as the base for relative paths.
  - **Why this design.** The Shortcut stays dumb (same 4 actions, every site). The server stays smart (decides A vs B per request). New site categories ("C-sites" — JS-rendered SPAs, paywalls with HTML markup) can be added server-side without a Shortcut change — every tester's phone keeps working unchanged. Trade: the Shortcut sends HTML on every request even when not needed (wasted bandwidth on A-sites). Acceptable — typical recipe page HTML is 50–500KB, fast on cellular.
  - **Gesture-prompt note.** The Shortcut's final action (Open URL) IS a user-initiated navigation, so iOS Safari typically allows the synchronous clipboard read on page load. If it denies (older iOS, strict permissions), the page still renders Import view in a clean state so the user can tap Paste manually. Soft floor: never worse than today's manual flow.
  - **Why HTML on the clipboard, not POST-from-Shortcut.** iOS Shortcuts can POST directly to an API, but our API is auth-gated by the user's Supabase session cookie, which the Shortcut doesn't have. Routing through a clipboard-and-open-URL handoff lets Safari (which DOES have the cookie) do the authenticated POST. No new auth surface, no API key embedded in tester Shortcuts. The clipboard is an OK carrier — if the user accidentally pastes elsewhere they see "https://...\n\n<html>..." which is recognizable junk, not sensitive data.
- **Smart paste — Paste tab detects `URL\n\nHTML` payloads (April 2026).** A manual-flow companion to smart_import that doesn't depend on the auto-flow's clipboard-permission and auth-redirect timing. When `handleImport()` runs with no overrides and the Paste textarea content matches `^\s*(https?:\/\/\S+)\s*\r?\n\s*\r?\n([\s\S]+)$`, the page splits the blob into URL + HTML and POSTs `{ url, html }` to `/api/import-recipe` instead of `{ text }`. Server already handles that shape (the smart_import path built it) — tries the URL fetch first for canonical og:image + content, falls through to the supplied HTML on B-sites. Result: the user shares a recipe via the iOS Shortcut, opens MyRecipe, taps 📥 Import → Paste tab, long-press → Paste, taps Import & Clean — and the recipe lands with image even on sites that block our scraper. The detection block is gated on `!urlToUse && !htmlToUse && textToUse` so smart_import (which passes overrides) and the URL tab path (which sets `importUrl`) are unaffected. **Why this exists alongside smart_import.** The auto-flow is friction-free when it works, but it's brittle: iOS Safari's clipboard-read permission prompt is gestured-gated, the `/secret?smart_import=1` URL trips the auth gate when the user isn't signed in on phone Safari, and Universal Clipboard sync between iPad and iPhone can confuse where the payload ends up. Smart paste sidesteps all of that — manual paste is a user gesture by definition, no permission prompt, and the user is already signed in by the time they navigate to the Paste tab. Trade: two extra taps (Import → Paste tab → long-press paste) versus one share-sheet tap. Worth it for reliability on B-sites until the smart_import auto-flow is proven across devices.
- **Clipboard auto-jump on Vault open (RETIRED April 2026).** Used to read the system clipboard on page load (`loadRecipes` → `readClipboardSmart()`), classify the contents (URL / URL+HTML / recipe text), and auto-route the user into the Import view pre-filled. Removed because (a) every page load triggered iOS's "Paste from <App>?" prompt, even when the clipboard wasn't a recipe — extra friction, surprising banner — and (b) the heuristic-based classification misfired on Bill's iOS Shortcut payload, routing him to the Paste tab pre-filled and losing the URL-fetch path that would have given him an image. The simpler model: user lands on the recipe list, taps 📥 Import → URL tab opens empty → user pastes URL themselves (long-press → Paste, or the keyboard's paste suggestion, or the explicit `📋 Paste` button next to the input) → tap Import. If URL fails, the existing auto-fallback flips them to Paste tab with the textarea focused. Helpers `readClipboardSmart()` and `htmlToCleanText()` are kept at module scope in `app/secret/page.js` because the smart_import handler still uses them; the auto-jump branches and `pendingHtml` state are gone. `openImportFromClipboard()` is now a one-liner that just opens Import on the URL tab — the function name is kept so Import button onClick handlers don't need editing, but it no longer reads the clipboard.
...
**Two sections, same data flag.** Both notes and videos use `favorites.is_in_vault=true` to mark them as "in Portfolio". The Portfolio query runs in parallel against `type='ai_answer'` (notes) and `type IN ('video_education','video_recipe')` (videos), populating two separate state slots. The render shows a **📺 Learning Videos** collapsible section first (sky-themed, closed by default; expands to a list rendered via the shared `<VideoItem>` component so playback stays in-app via inline iframe — same as Playbook), then the **5-bucket Notes accordion** below it (How to Prep / Cook / Season / Improve / Shop). Empty sections collapse — when the user has only notes, only the accordion shows; when only videos, only the video block shows.
...
## Recipe Cards heritage pass — Origin + Family Notes + handwriting font (May 2026)
...
**Naming canon (May 2026 rename).** The labels were re-shuffled so the heritage word lands on the heritage surface:

- **Vault `/secret`** — the working-recipe textarea is **"Notes"** (was "Family Notes"). A vault recipe is a working object — "less salt next time", "double the garlic", "store in airtight container" — and "Notes" reads honestly for that role. The DB column stays `personal_recipes.family_notes` (renaming a column would break too much) — only the user-facing label moved.
- **Card intro textarea** — **"Origin" (📜 amber)**. One-line introduction: where the recipe came from, who in the family makes it best, the memory attached to it. Eyebrow line reads "— where it came from". Placeholder leans into origin ("Grandma's recipe — she got it from her sister" / "Sunday dinner growing up").
- **Card dated-entries section** — **"Family Notes" (📝 stone)**. Every time you cook it, you add a dated entry. Stack of timestamped notes that builds over years. Eyebrow line reads "— each time you make it". This is where the *family* literally adds to the card across decades; promoting the heritage label here puts it where the ritual actually happens. (Was called "Cook Log" briefly when first shipped; renamed in the same May 2026 pass.)

The two surfaces on a Card (Origin + Family Notes) are deliberately different colors: amber for the introduction (the warm, pinned, one-time write) and stone for the running history (the page that gets written on). Together they read as *the story* (Origin) plus *the memories* (Family Notes).

**Visual — the page that's been written on.** The Family Notes block uses a stone palette with a faint horizontal-rule background drawn via a `repeating-linear-gradient` of 28px stripes at ~18% opacity — reads as index-card paper without screaming for attention. Entry text and the new-entry textarea both render in the **Caveat** handwriting font (Google Fonts, loaded via `next/font/google` in `app/layout.js` and exposed as the CSS variable `--font-caveat`). Caveat at 20px / 28px line-height has the warmth of pen-on-card without the legibility tradeoff of more cursive scripts. Entry dates render in a small uppercase tracking-wider style that looks like a date stamp above the handwritten note.

**Data model.** New table `recipe_cook_log` (migration `supabase/014_recipe_cook_log.sql`) — `id uuid, user_id uuid, recipe_id uuid (fk personal_recipes, cascade), entry_date date, notes text, created_at timestamptz`. The table name is historical (it was first shipped as "Cook Log") and stays — it's never user-visible. Composite index on `(user_id, recipe_id, entry_date desc, created_at desc)` matches the natural query shape (one card's history, newest first). RLS scoped to owner. Idempotent. Distinct from `personal_recipes.family_notes` (the Origin / Notes column — same DB field, different surfaces) — both stay live, and editing one doesn't touch the other.
...
**Why the font is loaded app-wide, not just on /cards.** `Caveat` is imported in `app/layout.js` alongside Geist and exposed as a CSS variable so it's available anywhere via `style={{ fontFamily: 'var(--font-caveat)' }}`. Recipes flow between Vault and Cards, and we want the option to use the same script font on Origin or other heritage surfaces later without re-importing.

**What's deliberately NOT in v1.**
- No multi-line "well-loved" / "cooked N times" stamp on the tile — the Family Notes section already implies that and a count metric on the tile would be premature optimization.
- No card-back / flip gesture. A subtle "↺" that turns the card around to show the entries is a future move; it adds animation surface area v1 doesn't need.
- No Origin font swap. Caveat is loaded for that purpose but only Family Notes uses it now — keeping Origin in the system font lets it stay scannable while the dated entries get the handwritten treatment. If the handwritten look lands well, extending to Origin is a one-line change.
...
- `/api/import-recipe` — parse/ingest external recipes. Accepts `{ url }` or `{ text }`. **YouTube support:** if `url` is a `youtu.be` / `youtube.com/watch` / `youtube.com/shorts` link, the route pulls title/channel/description/thumbnail via the YouTube Data API v3 and captions via `youtube-transcript`, then feeds the combined blob to Claude. Requires `YOUTUBE_API_KEY` in env. Falls back to description-only when captions are unavailable. Thumbnail becomes the recipe image. **Structured-data extraction (May 2026)** — when the page ships JSON-LD Recipe markup, `parseRecipeSchema()` extracts `prepTime` / `cookTime` / `totalTime` (ISO 8601 → minutes via `parseISODuration`), `recipeYield` (→ servings), and the nested `nutrition` object (`calories`, `proteinContent`, `carbohydrateContent`, `fatContent` → numbers via `parseNutrient`). Helpers `parseISODuration`, `parseNutrient`, `parseYieldServings` sit alongside the existing `extractImageUrl`. Direct extractions override anything Claude infers — sites that ship structured data get full timing + nutrition pulled in without an LLM round-trip needed for those fields. Returned in the JSON response as flat top-level keys (`prep_time_minutes`, `cook_time_minutes`, `total_time_minutes`, `calories`, `protein_g`, `carbs_g`, `fat_g`, `servings`).
...
- `/api/admin/video-action` (POST) — admin-only mutation endpoint. Bearer-token gated. Actions: `feature` / `unfeature` (flips `cooking_videos.is_featured`), `hide` / `unhide` (flips `is_hidden`, April 2026), and `update_meta` (inline title/channel edits — accepts `title` and/or `channel` in the body, refuses to clear the title). Returns `{ success: true, video: {...} }`. Uses `SUPABASE_SERVICE_ROLE_KEY` (falls back to anon if missing) so it can write regardless of RLS.

## Admin pages (`app/admin/`)

- `/admin/featured` — Featured curator for Chef TV (April 2026). Pared-down "what surfaces first" curator: just flips `cooking_videos.is_featured` on/off. Auth is double-gated — client-side check redirects non-admin to `/kitchen`, and the API routes also re-validate the Bearer token against `ADMIN_EMAIL`. UI: sticky search input (300ms debounce) + "⭐ only" checkbox; per-row a single ⭐ Feature / ☆ Unfeature button. Powers the ⭐ Featured chip on `/videos` Teach tab — see below.
- `/admin/library` — **Library curator for Chef TV (April 2026).** Bill's framing: "the videos are like books in a school's library — quality is key." Soft-hides videos that shouldn't surface and inline-edits titles/channels that came in mangled from ingestion. Filters: All / ✅ Visible / 🚫 Hidden, plus Any / 🍳 Has recipe / 🎓 No recipe (recipe filter is client-side because it depends on `video_metadata.ingredients` which the listing API joins in). Search matches title OR channel via `or(title.ilike,channel.ilike)`. Per-row actions: ⭐ Feature / ☆ Unfeature, 🚫 Hide / ↩️ Unhide, ✏️ Edit (inline title + channel — can't clear title). No hard delete (hidden rows stay in the DB so they're recoverable; Bill picked Hide-only for v1). No bulk actions. No re-extract — the standalone `backfill_video_metadata.mjs` script handles that. Hidden rows render at 70% opacity with a 🚫 chip and the Feature button hides on them (don't re-feature something you've hidden).
...
- **Cook** and **Learn Your Way** are *section names on MyKitchen*, not routes. Cook groups Recipe Vault + Recipe Cards + Meal Plan + Shopping List ("Recipes, meal plans & shopping, simplified by AI."). Learn Your Way groups Chef Jennifer + Chef TV + Your Library + My Playbook ("An AI-powered cooking school."). The Cook section was first labeled **Cooking Life** ("Your recipes, your plan, your essentials.") — renamed to **Cook** because a single verb reads as a clearer counterpart to a learning section, and the "simplified by AI" tag telegraphs the differentiator. The Learn-side traveled **Cooking School** → **Learning Journey** ("Your classrooms, your library, your practice book.") → **Learn Your Way** ("An AI-powered cooking school.") for the same reason. The "Guides" tile also picked up the friendlier name **"Your Library"** in the Learn Your Way pass (route stays `/guides`). There are no `/cook` or `/learn-your-way` URLs and there shouldn't be — the hub already does the framing.
...
**`?next=` post-sign-in redirect (April 2026).** Auth-gated pages can pass a `next` query param to `/login` so the user lands back on the URL they were trying to reach (instead of always `/kitchen`). The chain: auth gate redirects to `/login?next=/secret?import=…` → `/login` reads `next` and includes it in the OAuth `redirectTo` (`/auth/callback?next=…`) → `/auth/callback` preserves `next` when forwarding to `/auth/confirm` → `/auth/confirm` redirects to `next` (validated as same-origin relative path, falls back to `/kitchen` if missing/invalid). `/secret`'s auth gate, `/import`'s redirect-to-login, and any future deep-link entry should all populate `next` with `pathname + search` so the round-trip is transparent. **Why this matters for the iOS Shortcut path:** when the Shortcut's Open URL action opens `/secret?smart_import=1` in a context that doesn't share Safari's session (Safari View Controller has its own ephemeral cookies), the user has to sign in. Without `next` they'd land on `/kitchen` and lose the import URL; with it, sign-in returns to `/secret?smart_import=1`, the clipboard payload is still there, and the import fires. Open-redirect protection: only paths starting with `/` (and not `//`) are accepted.
