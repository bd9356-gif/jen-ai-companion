...
- **Clipboard auto-jump on Vault open (April 2026).** Bill's pivot from the iOS Shortcut path: the Shortcut UX was too fiddly to set up (URL Encode action, Open URL action, share-sheet enrollment), and the existing copy-paste flow already worked. To collapse the copy-paste path from 4 taps to 2, `loadRecipes` now reads `navigator.clipboard.readText()` after the auth load and branches on what it finds:
