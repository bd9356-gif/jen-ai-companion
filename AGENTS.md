...
- **Tags are organized into three labelled groups** (`TAG_GROUPS` in `app/secret/page.js`) totaling 15 curated suggestions: **🍽 Meal** (breakfast, lunch, dinner, dessert, side, snack), **🥩 Protein** (chicken, beef, fish, veg), **✨ Style** (quick, comfort, healthy, baking, holiday). `CURATED_TAGS` is the flat union, used to separate curated chips from custom tags in the renderer. Lunch was added in the same fix as the Category restore.
