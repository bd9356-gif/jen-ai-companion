...
**Data model — no migration.** Reuses the existing `favorites.is_in_vault` boolean (already in active use for `video_education` saves on `/secret`). For `type = 'ai_answer'` rows: `is_in_vault = true` ⇒ shows in Portfolio, `false` ⇒ Playbook-only. Promoting/demoting is a single column UPDATE; nothing else moves. Same `favorites` row, two surfaces.
