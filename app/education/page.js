...
export default function EducationPage() {
  ...
  async function loadSaved(userId) {
    const { data } = await supabase.from('saved_education_videos').select('video_id').eq('user_id', userId)
    setSavedIds(new Set((data || []).map(s => s.video_id)))
  }

  async function loadMetadata(videoId) {
    if (metadata[videoId] !== undefined) return
    const { data } = await supabase
      .from('education_video_metadata').select('*').eq('video_id', videoId).maybeSingle()
    setMetadata(prev => ({ ...prev, [videoId]: data || null }))
  }

  async function toggleSave(videoId) {
    if (!user) return
    if (savedIds.has(videoId)) {
      await supabase.from('saved_education_videos').delete().eq('user_id', user.id).eq('video_id', videoId)
      setSavedIds(prev => { const n = new Set(prev); n.delete(videoId); return n })
    } else {
      await supabase.from('saved_education_videos').insert({ user_id: user.id, video_id: videoId })
      ...
      <header className="bg-white border-b border-gray-100 sticky top-0 z-10">
        <div className="max-w-4xl mx-auto px-4 pt-4 pb-3">
          <div className="flex items-center gap-2 mb-1">
            ...
            <h1 className="text-lg font-bold text-gray-900">🎓 Chef Education</h1>
          </div>
          <p className="text-xs text-gray-400 mb-3">Recipe-first channels — ingredients & instructions included</p>
          ...
          <div className="flex gap-2 mb-3">
            {[['all','All'],['recipe','🍳 Has Recipe'],['summary','📝 Summary Only']].map(([val, label]) => (
              <button key={val} onClick={() => { setFilter(val); setShowCount(10) }}
                className={`px-3 py-1.5 rounded-full text-xs font-semibold transition-colors ${
                  filter === val ? 'bg-orange-600 text-white' : 'bg-gray-100 text-gray-600 hover:bg-orange-50'}`}>
                {label}
              </button>
            ))}
          </div>
                          ...
                          {meta === undefined ? (
                            ...
                            <>
                              ...
                              {!hasRecipe && !meta.ai_summary && (
                                <p className="text-sm text-gray-400 italic">No recipe details available.</p>
                              )}
                            </>
                          )}
                        </div>
