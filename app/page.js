...
export default function HomePage() {
  ...
  {user ? (
        ...
        <main className="flex-1 px-6 pt-4 pb-12 flex flex-col items-center text-center">
...
{userName ? (
                <>
                  <h1 className="text-2xl font-bold text-white mb-1 drop-shadow">Welcome back, {userName}!</h1>
                <p className="text-orange-200 text-sm drop-shadow">Your kitchen is ready.</p>
  </>
            ) : (
                            <>
                              <h1 className="text-2xl font-bold text-white mb-1 drop-shadow">Welcome to MyRecipe Companion</h1>
                <p className="text-orange-200 text-sm drop-shadow">1,464 recipes powered by AI.</p>
              </>
            )}
</div>
              </div>
        <a href={user ? '/kitchen' : '/kitchen'}
        ...
        <div className="w-full mt-4 border border-gray-200 rounded-2xl overflow-hidden">
          ...
          {STATS.map(({ stat, label }, i) => (
                          <div key={label} className={`p-5 text-center ${i % 2 === 0 ? 'border-r border-gray-200' : ''} ${i >= 2 ? 'border-t border-gray-200' : ''}`}>
        ...
        <a href="/about" className="mt-8 text-sm text-gray-400 hover:text-gray-600">About MyRecipe Companion →</a>
  </main>
  </div>
  )
}
