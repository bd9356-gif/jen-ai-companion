...
// populated — it shows the feature exists without the user having to
// discover it cold. NOTE: favoriting is decoupled from Meal Plan. Day-one
// Meal Plan is intentionally empty; the user fills it by tapping 📅 Meal
// Plan on a recipe themselves. Heart = preference signal, calendar =
// "I'm cooking this".
...
export const STARTER_RECIPES = [
  {
    ...
    photo_url: 'https://lexiscleankitchen.com/wp-content/uploads/2025/03/aglio-e-olio-7-736x1104.jpg',
    is_favorite: true,
  },

  {
    ...
    family_notes: "Welcome — this starter's here to get you cooking. Yours to make your own.",
    photo_url: 'https://th.bing.com/th/id/OIP.iMs_liuQdGaKqlLjg8MepgHaHa?w=185&h=184&c=7&r=0&o=7&pid=1.7&rm=3',
  },

  {
    ...
    family_notes: "Welcome — this starter's here to get you cooking. Yours to make your own.",
    photo_url: 'https://www.erinliveswhole.com/wp-content/uploads/2022/10/TomatoSoupandgrilledcheese-14-768x1152.jpg',
  },

  {
    ...
    family_notes: "Welcome — this starter's here to get you cooking. Yours to make your own.",
    photo_url: 'https://tse1.mm.bing.net/th/id/OIP.9ftDdehbasvlWcAhmcyC6wHaHa?rs=1&pid=ImgDetMain&o=7&rm=3',
  },
...
// Titles of starter recipes that should be flagged is_favorite=true.
// Single source of truth for:
//   1. the recipe seed (data above carries is_favorite: true on these)
//   2. the backfill seeder (reads this list to retro-flag existing rows
//      whose seed missed the column write)
//
// The Meal Plan seeder used to read this list too, but it was removed
// (April 2026) — favoriting and meal-planning are separate gestures
// and shouldn't be coupled at seed-time. Day-one Meal Plan is empty
// by design. See the long comment in app/kitchen/page.js where
// seedMealPlanOnce used to live.
//
// Bump STARTER_BACKFILL_VERSION below to re-run the backfill (e.g. if
// this list grows or we want to re-flag rows whose is_favorite got
// cleared by some other process).
export const FAVORITE_STARTER_TITLES = [
  'Spaghetti Aglio e Olio',
  'Brown Butter Chocolate Chip Cookies',
]

// Version flag for the one-time backfill that walks existing seeded
// starter rows and writes is_favorite=true on the favorite-titled ones.
// Why this exists: in the original v1/v2 ship, the recipe seed ran
// before the is_favorite column was reliably available on the
// personal_recipes schema (or PostgREST hadn't reloaded its cache),
// so some users have starters in their Vault but with is_favorite=false.
// The backfill fixes those accounts on the next MyKitchen visit.
// Idempotent — UPDATE is a no-op if already true.
export const STARTER_BACKFILL_VERSION = 'v1'

// Version flag for the one-time backfill that walks existing seeded
// starter rows and writes the canonical photo_url onto each one.
// Why this exists: starters were originally seeded with empty photo_url
// values (we hadn't picked images yet). When images were added to
// STARTER_RECIPES later, existing users were stuck with photo-less
// starters because the seeder only runs on fresh signups. This backfill
// matches by title + the "Welcome —" family_notes prefix and updates
// photo_url ONLY on rows where it's currently empty — so a user who
// changed the photo on a starter never gets overwritten. Bump this
// version to re-run (e.g. if we swap photos later).
