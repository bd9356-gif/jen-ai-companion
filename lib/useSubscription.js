...
export const LIMITS = {
  ...
  premium: { chefJen: 5, imports: Infinity, photos: 5, recipes: Infinity },
  ...
  useEffect(() => {
    async function check() {
      ...
      try {
        ...
        if (entitlements['pro']) { setTier('pro'); setLoading(false); return }
        else if (entitlements['premium']) { setTier('premium'); setLoading(false); return }
      } catch (err) {
        console.error('[useSubscription]', err)
      }
      // RevenueCat returned free or failed — check Supabase as fallback
      try {
        const { data: { session } } = await supabase.auth.getSession()
        if (session?.user) {
          const { data } = await supabase
        ...
        console.error('[useSubscription fallback]', err)
        setTier('free')
      }
      setLoading(false)
    }
